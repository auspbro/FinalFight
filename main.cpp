...
void main()
{
	...
	mciSendString( "open ./res/Sounds/选人.mp3 alias xuan", NULL, 0, NULL );
...
void Init()
{
	...
	for ( i = 0; i < 2; i++ )
	{
		...
		getimage( &FOOD::img[i], i * 80, 720, 80, 80 );		// 鸡腿
	}
	SetWorkingImage();
...
void Dispatch_role( int role )
{
	if ( role > 22 )
	{
		...
		switch ( role % 10 )
		{
			...
			default:	MessageBox( NULL, "该玩家还没创建出来!", "警告", MB_OK );	break;
		}
		return;
	}
...
void Show_Information()
{
	...
	for ( i = 0; i < PLAYER::player_num; i++ )
	{
			...
			outtextxy( Player[i].x + 180, 44, "命/关:" );
...
void Check_boss( bool& game )
{
		 ...
		 Map.map_id == 4 && Map.part[4] == 5 )
	{
		...
		if ( b_sum == 0 )
		{
			if ( Map.map_id == 4 && Map.part[4] == 7 )
			{
				MessageBox( GetHWnd(), "谢谢惠顾！see you", "Hello Kitty", MB_OK );
				game = false;
				return;
			}
	...
	{
		mciSendString( "open ./res/Sounds/玩家死光.mp3 alias ww", 0, 0, 0 );
		mciSendString( "play ww", 0, 0, 0 );

		if ( IDYES == MessageBox(GetHWnd(), "是否要花一毛钱续命？", "note", MB_YESNO) )
		{
			for ( int i = 0; i < PLAYER::player_num; i++ )
			{
...
void Role::Jump_knee( int dir, int player )
{
	...
	{
		case 9:		jk = 1;	break;
		case 19:	jk = 2;	SetSound( NULL, "落地", "s17" );	break;
		case 21:
			Drtx	= 0;
			temp_x = 0;
			n_jk	= 0;
	...
	for ( int i = 0; i < 2; i++ )
	{
		for ( int x = 0; x < 10; x++ )// [1][][]为右、 [0][][]为左
		{
			if ( x < 4 )
			{
				...
				getimage( &b_blow[1][x][i], x * 80, 1280 + i * 80, 80, 80 );	// 大招不分左右
			...
			if ( x < 6 )	// 8、9之间不连接！！
			{
				...
				getimage( &kick_down[1][x][i], x * 80, 2576 + i * 80, 80, 80 );		// 方向有点不一样
...
void Kady::Dragon_fist( int dir, int player )
{
	...
	if ( n_df == 25 )	df = 7;
	if ( n_df == 4 )
		SetSound( NULL, "dragon_fist声", "s10" );
...
void Heye::Light_wave( int dir, int player )
{
	...
	if ( n_lw == 4 )
		SetSound( NULL, "连环腿1", "h6" );
	if ( n_lw == 16 )
		SetSound( NULL, "连环腿2", "h7" );
...
void Heye::Dragon_fist( int dir, int player )
{
	...
	{
		case 2:		df = 1;		break;
		case 4:		df = 2;	SetSound( NULL, "倒勾脚", "h9" );	break;
		case 6:		df = 3;		break;
		case 12:	df = 4;		break;
		case 15:	df = 5;		break;
		case 18:	df = 6;		break;
	...
	for ( int i = 0; i < 2; i++ )
	{
		for ( int x = 0; x < 6; x++ )
		{
			if ( x < 4 )
			{
				...
				getimage( &throw_role[1][x][i], x * 80, 2880 + i * 80, 80, 80 );	// 合成时搞反了
			}
			...
			getimage( &kick_down[0][x][i], x * 80, 2080 + i * 80, 80, 80 );		// 9 方向有点不一样
...
void Jack::Dragon_fist( int dir, int player )
{
	...
	switch ( n_df )
	{
		...
		case 11:	df = 3;	temp_y += 30;	SetSound( NULL, "双脚跳踢", "j7" );	break;
		case 18:	df = 4;					break;
		case 21:
			n_df = df = 0;
			n_a = a = 0;
...
void Jack::Throw_role( int dir, int player )
{
	...
	switch ( n_tr )
	{
		case 4:		tr = 2;	SetSound( NULL, "甩人", "j8" );	break;
...
{
	IMAGE img;
	loadimage( &img, "./res/Images/猴仔.jpg", 828, 2748 );
	SetWorkingImage( &img );
...
void Small_Monkey::Kick_away( int id, const int& player )
{
	...
	if ( Emeny[id].be_kick_counter == 4 )
	{
		...
		{
			// 进坑
			PlaySound( "./res/Sounds/敌人倒地.wav", NULL, SND_ASYNC );
			if ( Map.map_id == 2 && Check_hole(PE_emeny, id) == _IN
				 || Check_lift(PE_emeny, id) == _IN )
			{
				n_ka = 51;
...
{
	IMAGE img;
	loadimage( &img, "./res/Images/小呆.jpg", 800, 3000 );
	SetWorkingImage( &img );
...
{
	IMAGE img;
	loadimage( &img, "./res/Images/大呆.jpg", 800, 2000 );
	SetWorkingImage( &img );
...
{
	IMAGE img;
	loadimage( &img, "./res/Images/妹子.jpg", 800, 2000 );
	SetWorkingImage( &img );
...
{
	IMAGE img;
	loadimage( &img, "./res/Images/廋老头.jpg", 800, 2000 );
	SetWorkingImage( &img );
...
{
	IMAGE img;
	loadimage( &img, "./res/Images/大个子野人.jpg", 800, 2000 );
	SetWorkingImage( &img );
...
void B_FIVE::Attack( int id, const int& player )
{
	...
	default: MessageBox( NULL, "B_FIVE::Attack()出错", "ds", MB_OK );	break;
	}
	...
	for ( int i = 0; i < BOSS::Boss_num; i++ )
		if ( !Boss[i].die && !Boss[i].appear )
		{
			...
			switch (part)
			{
				case 2:
					{
						...
						if ( i + 1 == BOSS::Boss_num )
						{
							char c[12][3] = { "妖","怪","，","杀","我","小","弟","快","拿","命","来","！" };
							mciSendString( "open ./res/Sounds/dada.mp3 alias da", 0, 0, 0 );
							mciSendString( "play da repeat", 0, 0, 0 );
				...
				default:
					MessageBox( GetHWnd(), "boss_define.cpp的Dispatch_five_boss()出错", "not", MB_OK );
					break;
			}
		}
}
